...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    header("Location: manage_advisors.php?msg=Invalid advisor ID.");
    exit();
}
...
$stmt = $conn->prepare("
    ...
    WHERE u.user_id = ? AND u.role = 'advisor'
");
$stmt->bind_param("i", $advisor_id);
$stmt->execute();
$result = $stmt->get_result();
if ($result->num_rows === 0) {
    header("Location: manage_advisors.php?msg=Advisor not found.");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($matrix) || empty($name) || empty($utm_email) || empty($phone) || empty($faculty) || empty($department)) {
    ...
    if (empty($errors)) {
        ...
        $stmt->bind_param("si", $matrix, $advisor_id);
        $stmt->execute();
        ...
        if (empty($errors)) {
            ...
            $stmt->bind_param("si", $utm_email, $advisor_id);
            $stmt->execute();
            if ($stmt->get_result()->num_rows > 0) {
    ...
    if (!empty($errors)) {
        ...
        if (!empty($password)) {
            ...
            $stmt->bind_param("ssssssi", $matrix, $name, $utm_email, $second_email, $phone, $hashed, $advisor_id);
        } else {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: manage_advisors.php?msg=Advisor updated successfully.");
            exit();
        } else {
...
<head>
    ...
    <title>Edit Advisor - Admin Portal</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Poppins', sans-serif; }
        ...
        .form-card { background: white; border-radius: 25px; padding: 35px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); max-width: 3000px; }
    ...
    <div class="form-card">
        <form method="POST">
            <div class="row-custom">
                <div class="form-group"><label>Matrix Number</label><input type="text" name="matrix" value="<?php echo htmlspecialchars($advisor['matrix_number']); ?>" required></div>
                <div class="form-group"><label>Full Name</label><input type="text" name="name" value="<?php echo htmlspecialchars($advisor['user_name']); ?>" required></div>
                <div class="form-group"><label>UTM Email</label><input type="email" name="utm_email" value="<?php echo htmlspecialchars($advisor['utm_email']); ?>" required></div>
                <div class="form-group"><label>Second Email</label><input type="email" name="second_email" value="<?php echo htmlspecialchars($advisor['second_email']); ?>"></div>
                <div class="form-group"><label>Phone Number</label><input type="text" name="phone" value="<?php echo htmlspecialchars($advisor['phone']); ?>" required></div>
                <div class="form-group"><label>Faculty</label>
                    <select name="faculty" required>
                        <option value="Faculty Of SPACE" <?php echo $advisor['faculty'] == 'Faculty Of SPACE' ? 'selected' : ''; ?>>Faculty Of SPACE</option>
                        <option value="Faculty Of Computer Science" <?php echo $advisor['faculty'] == 'Faculty Of Computer Science' ? 'selected' : ''; ?>>Faculty Of Computer Science</option>
                    </select>
                </div>
                <div class="form-group"><label>Department</label>
                    <select name="department" required>
                        <option value="Computer Science" <?php echo $advisor['department'] == 'Computer Science' ? 'selected' : ''; ?>>Computer Science</option>
                        <option value="Mathematics" <?php echo $advisor['department'] == 'Mathematics' ? 'selected' : ''; ?>>Mathematics</option>
                        <option value="Sports Science" <?php echo $advisor['department'] == 'Sports Science' ? 'selected' : ''; ?>>Sports Science</option>
                        <option value="Electrical Engineering" <?php echo $advisor['department'] == 'Electrical Engineering' ? 'selected' : ''; ?>>Electrical Engineering</option>
                        <option value="Pengajian Islam" <?php echo $advisor['department'] == 'Pengajian Islam' ? 'selected' : ''; ?>>Pengajian Islam</option>
                    </select>
                </div>
                <div class="form-group"><label>New Password (leave blank to keep current)</label><input type="password" name="password"></div>
            </div>
            <button type="submit" class="btn-submit"><i class="bi bi-save"></i> Update Advisor</button>
        </form>
    </div>
</div>
<script>
