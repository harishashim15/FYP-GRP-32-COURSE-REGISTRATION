...
// Fetch admin name and password from users table
$stmt = $conn->prepare("SELECT user_name, password FROM users WHERE user_id = ? AND role = 'admin'");
...
if (!$admin) {
    // Not an admin or user not found – destroy session and redirect
    session_destroy();
    header("Location: ../login.html");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (str_starts_with($stored_password, '$2y$')) {
        $valid = password_verify($current_password, $stored_password);
    } else {
    ...
    if (!$valid) {
        ...
        $update_stmt->bind_param("si", $hashed, $admin_id);
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Poppins', sans-serif; }
        ...
        .page-header {
            background: #f7f2ee; border-radius: 25px; padding: 35px 40px;
            display: flex; justify-content: space-between; align-items: center;
        ...
        .input-wrapper .form-control {
            border: 1.5px solid #e0d6d6; border-radius: 14px;
            padding: 13px 50px 13px 20px; font-size: 14px; background: #fdfafa;
            width: 100%;
        }
        ...
        .requirements {
            background: #fdf8f8; border: 1px solid #f0e5e5;
            border-radius: 14px; padding: 18px 20px; margin-bottom: 25px;
        }
        ...
        .btn-save {
            ...
            color: white; border: none; padding: 13px 35px;
        ...
        .btn-cancel {
            background: white; color: #670019; border: 1.5px solid #e0d6d6;
            padding: 13px 35px; border-radius: 14px; font-size: 15px; font-weight: 600;
            cursor: pointer; text-decoration: none; display: inline-flex; align-items: center; gap: 8px;
        }
        .btn-cancel:hover { background: #fff7ef; border-color: #f4a000; color: #670019; }
        .alert-custom {
        ...
        .tip-card {
            ...
            box-shadow: 0px 4px 15px rgba(0,0,0,0.05);
            max-width: 340px; height: fit-content;
        }
        ...
        .tip-icon {
            ...
            background: #fff2cc; color: #d48a00;
...
<div class="sidebar">
    ...
    <div class="menu">
        ...
        <a href="admin_changepassword.php" class="active"><i class="bi bi-key-fill"></i> Change Password</a>
    </div>
            ...
            <div class="form-card">
                ...
                <?php if ($message): ?>
                    <div class="alert-custom alert-<?php echo $message_type; ?>">
                        <i class="bi bi-<?php echo $message_type == 'success' ? 'check-circle-fill' : 'exclamation-triangle-fill'; ?> fs-5"></i>
                ...
                <form method="POST" action="">
                    ...
                    <div class="d-flex gap-3 flex-wrap mt-2">
                        ...
                        <a href="admin_dashboard.php" class="btn-cancel"><i class="bi bi-x-circle"></i> Cancel</a>
                    </div>
                </form>
            </div>
        </div>
