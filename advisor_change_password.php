...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Poppins', sans-serif; }
        ...
        .form-card { background: white; border-radius: 25px; padding: 40px; box-shadow: 0px 4px 15px rgba(0,0,0,0.05); max-width: 700px; }
        ...
        .input-wrapper .form-control { border: 1.5px solid #e0d6d6; border-radius: 14px; padding: 13px 50px 13px 20px; font-size: 14px; background: #fdfafa; }
...
<body>
        ...
        <div class="d-flex gap-4 flex-wrap">
            <div class="form-card flex-grow-1">
                ...
                <form method="POST" id="passwordForm">
                    <div><label class="form-label">Current Password</label><div class="input-wrapper"><input type="password" class="form-control" name="current_password" id="currentPassword" placeholder="Enter your current password" required><button type="button" class="toggle-eye" onclick="toggleVisibility('currentPassword', this)"><i class="bi bi-eye"></i></button></div></div>
                    <div><label class="form-label">New Password</label><div class="input-wrapper"><input type="password" class="form-control" name="new_password" id="newPassword" placeholder="Enter your new password" oninput="checkStrength(this.value); checkRequirements(this.value);" required><button type="button" class="toggle-eye" onclick="toggleVisibility('newPassword', this)"><i class="bi bi-eye"></i></button></div></div>
                    <div class="strength-bar-wrapper"><div class="strength-label" id="strengthLabel">Password strength: —</div><div class="strength-bar"><div class="strength-fill" id="strengthFill"></div></div></div>
                    <div class="requirements"><p><i class="bi bi-info-circle me-1"></i> Password Requirements</p>
                    ...
                    <div><label class="form-label">Confirm New Password</label><div class="input-wrapper"><input type="password" class="form-control" name="confirm_password" id="confirmPassword" placeholder="Re-enter your new password" oninput="checkMatch()" required><button type="button" class="toggle-eye" onclick="toggleVisibility('confirmPassword', this)"><i class="bi bi-eye"></i></button></div><div id="matchMsg" style="font-size:12px; margin-top:-14px; margin-bottom:16px;"></div></div>
                    <div class="d-flex gap-3 flex-wrap mt-2"><button type="submit" class="btn-save"><i class="bi bi-shield-check"></i> Update Password</button><a href="advisor_dashboard.php" class="btn-cancel"><i class="bi bi-x-circle"></i> Cancel</a></div>
                </form>
            </div>
    ...
    <script>
        function toggleVisibility(inputId, btn) { const input = document.getElementById(inputId); const icon = btn.querySelector('i'); if (input.type === 'password') { input.type = 'text'; icon.className = 'bi bi-eye-slash'; } else { input.type = 'password'; icon.className = 'bi bi-eye'; } }
        function checkStrength(val) { const fill = document.getElementById('strengthFill'); const label = document.getElementById('strengthLabel'); let score = 0; if (val.length >= 8) score++; if (/[A-Z]/.test(val)) score++; if (/[a-z]/.test(val)) score++; if (/[0-9]/.test(val)) score++; if (/[^A-Za-z0-9]/.test(val)) score++; const levels = [{ w: '0%', bg: '#eee', text: '—' }, { w: '20%', bg: '#e74c3c', text: 'Very Weak' }, { w: '40%', bg: '#e67e22', text: 'Weak' }, { w: '60%', bg: '#f1c40f', text: 'Fair' }, { w: '80%', bg: '#2ecc71', text: 'Strong' }, { w: '100%', bg: '#27ae60', text: 'Very Strong' }]; const l = val.length === 0 ? levels[0] : levels[score]; fill.style.width = l.w; fill.style.background = l.bg; label.textContent = `Password strength: ${l.text}`; }
        function checkRequirements(val) { const checks = { 'req-length': val.length >= 8, 'req-upper': /[A-Z]/.test(val), 'req-lower': /[a-z]/.test(val), 'req-number': /[0-9]/.test(val), 'req-special': /[^A-Za-z0-9]/.test(val) }; for (const [id, passed] of Object.entries(checks)) { const el = document.getElementById(id); const icon = el.querySelector('i'); if (passed) { el.classList.add('met'); icon.className = 'bi bi-check-circle-fill'; } else { el.classList.remove('met'); icon.className = 'bi bi-circle'; } } }
        function checkMatch() { const np = document.getElementById('newPassword').value; const cp = document.getElementById('confirmPassword').value; const msg = document.getElementById('matchMsg'); if (cp === '') { msg.textContent = ''; return; } if (np === cp) { msg.innerHTML = '<span style="color:#2e7d32;"><i class="bi bi-check-circle-fill"></i> Passwords match</span>'; } else { msg.innerHTML = '<span style="color:#b00020;"><i class="bi bi-x-circle-fill"></i> Passwords do not match</span>'; } }
